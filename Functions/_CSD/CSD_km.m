...
%
function [X, Y] = CSD_km(Data, G, H, lambda, head)
...
G=G+(eye(length(G))*lambda);           % add smoothing constant to diagonale
Gi = inv(G);                           % compute G inverse
TC = sum(Gi,2);                        % compute sums for each row
...
if license('test','Distrib_Computing_Toolbox')
    ...
    parfor p = 1:nPnts
        ...
        Crep = repmat(C,[1,nElec]);
        X(:,p) = sum(Crep .* H') / head;   % compute all CSDs and scale to head size
        if mynargout > 1;
            Y(:,p) = c0 + sum(Crep .* G');   % ... compute all SPs
        end;
    end;
else
    for p = 1:nPnts
