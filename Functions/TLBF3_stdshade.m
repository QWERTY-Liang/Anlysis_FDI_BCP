...
astd = nanstd(amatrix,[],1)/3; % to get std shading 这里改变缩小了阴影
% astd = nanstd(amatrix,[],1)/sqrt(size(amatrix,1)); % to get sem shading

if exist('alpha','var')==0 || isempty(alpha) 
    fillOut = fill([F fliplr(F)],[amean+astd fliplr(amean-astd)],acolor,'linestyle','none','HandleVisibility','off');%这里把阴影关闭
    acolor='k';
else
    fillOut = fill([F fliplr(F)],[amean+astd fliplr(amean-astd)],acolor, 'FaceAlpha', alpha,'linestyle','none','HandleVisibility','off');%这里把阴影关闭
end
...
hold on;
lineOut = plot(F,amean, 'color', acolor,'linewidth',1.25); %% change color or linewidth to adjust mean line
