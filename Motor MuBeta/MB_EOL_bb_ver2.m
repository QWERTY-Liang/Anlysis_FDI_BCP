...
%测试无baseline情况
 %EEG = pop_loadset([exp.finalpath 'csd_cICAri' epoch '_' exp.filterLab 'aac' exp.name num2str(sub) '.set']);
...
epoch_limits_msTG = [-188*8 188*4];    % Target-locked epoch. We could use the same windows as were used for artifact rejection, or we might want different windows here - if so, don't forget that artifacts were only checked for in the window specified in IdentifyBadTrials!
...
ff = find((F>8 & F<21) | (F>22 & F<30)); % the indices of F that cover the spectral range/band of interest. Let's say we're interested in Mu and Beta bands combined. Note here I'm avoiding the SSVEP frequency (18.75hz in this example)
Ts = [-188*8:47:188*4]; % in msec, centered on what times do you want to measure spectral amplitude? i.e., where to center each consecutive window in time
...
% colorbar
time_indices = find(Ts >= -1200 & Ts <= 300);  % Change the range as required
n_subplots = 12;  % Number of subplots
time_points = linspace(-1200, 300, n_subplots);  % Select 12 time points between -100 and 1200 ms
