...
for sub=1:6% 每个人分别看
    ...
    fftlen = round(fs/21.25*12); % Window of how many sample points? If there is an SSVEP involved, whether or not you are interested in analyzing it, it is good to have all power related to the SSVEP isolated in a single frequency bin. This happens when you choose a window length that is an integer number of SSVEP cycles.
    ...
    Ts = [-950:20:2700];%[-188*4:47:188*12]; % in msec, centered on what times do you want to measure spectral amplitude? i.e., where to center each consecutive window in time
    ...
    for i=1:length(AllBehaviour)
        %select trials
        if AllBehaviour(i,8)==1 && AllBehaviour(i,9)==1%&& AllBehaviour(i,3)==0.14%&& AllBehaviour(i,4)==4% && AllBehaviour(i,4)==44
            selected_trials_1(i)=1;
        elseif AllBehaviour(i,8)==2 && AllBehaviour(i,9)==2%&& AllBehaviour(i,3)==0.14%&& AllBehaviour(i,4)==4%&& AllBehaviour(i,4)==44
    ...
    trl2=find(selected_trials_2==1);
     %low
selected_trials_11 = zeros(length(AllBehaviour),1);  % left C4-C3
selected_trials_22 = zeros(length(AllBehaviour),1);  % right C3-C4
count=0;
% switch%%%%%%%%%%calculate correct first
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour)
            %select trials
            if AllBehaviour(i,8)==1  && AllBehaviour(i,9)==2%&&AllBehaviour(i,3)==0.07% AllBehaviour(i,4)==44%&& AllBehaviour(i,3)==0.07
                selected_trials_11(i)=1;
            elseif AllBehaviour(i,8)==2  && AllBehaviour(i,9)==1%&& AllBehaviour(i,3)==0.07%AllBehaviour(i,4)==44%&& AllBehaviour(i,3)==0.07
    ...
    %% compute
    %
    avMB1(:,:,sub) = mean(STFT(:,:,trl1),3);
    avMB2(:,:,sub) = mean(STFT(:,:,trl2),3);
    avMB11(:,:,sub) = mean(STFT(:,:,trl11),3);% wrong-left
    avMB22(:,:,sub) = mean(STFT(:,:,trl22),3);% wrong-right

end
%%%%手动命名保存
% 保存 avMB1 和 avMB2 变量到指定文件路径 
%名称规则 波段_切分方法_分类方法_肌肉/条件
save('G:\My Drive\Phd\Stage1\BCPvsFDI\E1data_polit\Anlysis_v2\Motor MuBeta\MuBeta_SL_CR_CW.mat', 'avMB1', 'avMB2', 'avMB11', 'avMB22');
...
num_plots = 15;
time_points = linspace(-1000,1500, num_plots);  % in milliseconds
