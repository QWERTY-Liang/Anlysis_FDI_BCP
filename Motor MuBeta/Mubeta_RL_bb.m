...
epoch_limits_msTG =  [-188*4 188];    % Response-locked epoch; again using integer number of SSVEP cycles nt windows here - if so, don't forget that artifacts were only checked for in the window specified in IdentifyBadTrials!
...
trange = find(Ts>=-100,1); % pick a time just before response
figure;
