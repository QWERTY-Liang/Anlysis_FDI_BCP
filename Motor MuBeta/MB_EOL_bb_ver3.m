    ...
    for e=2:3 % 2 是RL;3是EOL\
        ...
        for sub=1:6% 每个人分别看
            ...
            for i=1:length(AllBehaviour)
                %select trials
                if AllBehaviour(i,8)==1 && AllBehaviour(i,9)==1%&& AllBehaviour(i,3)==0.14%&&AllBehaviour(i,4)==4% &&&& AllBehaviour(i,3)==0.14%&& AllBehaviour(i,9)==1%&& AllBehaviour(i,3)==0.14%&& AllBehaviour(i,4)==4% && AllBehaviour(i,4)==44
                    selected_trials_1(i)=1;
                elseif AllBehaviour(i,8)==2&& AllBehaviour(i,9)==2%&& AllBehaviour(i,3)==0.14%&&AllBehaviour(i,4)==4%&& AllBehaviour(i,3)==0.14 %&& AllBehaviour(i,9)==2%&& AllBehaviour(i,3)==0.14%&& AllBehaviour(i,4)==4%&& AllBehaviour(i,4)==44
        ...
        save(['G:\My Drive\Phd\Stage1\BCPvsFDI\E1data_polit\Anlysis_v2\Motor MuBeta\' fbandname '_' epoch '_CW.mat'], 'avMB1', 'avMB2', 'avMB11', 'avMB22');
    end
end
%%%%手动命名保存
