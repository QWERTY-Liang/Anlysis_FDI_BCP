...
addpath(genpath(exp.finalpath));
 EEG = pop_loadset([exp.finalpath 'nobaseline_RL_bTLalldata.set']);
 load TL_AllBehaviour_RL_nobaseline.mat 

AllBehaviour_SL_bb=AllBehaviour_RL_bb;
%% STFT prepare
fs=EEG.srate;%512
t=EEG.times;
erp=EEG.data;

epoch_limits_msTG = [-188*4 188];    % Target-locked epoch. We could use the same windows as were used for artifact rejection, or we might want different windows here - if so, don't forget that artifacts were only checked for in the window specified in IdentifyBadTrials!
...
Ts = [-188*4:47:188]; % in msec, centered on what times do you want to measure spectral amplitude? i.e., where to center each consecutive window in time
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_SL_bb)
            %select trials
            if AllBehaviour_SL_bb(i,8)==1 && AllBehaviour_SL_bb(i,9)==2
                selected_trials_11(i)=1;
            elseif AllBehaviour_SL_bb(i,8)==2 && AllBehaviour_SL_bb(i,9)==1
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_SL_bb)
            %select trials
            if AllBehaviour_SL_bb(i,8)==1 && AllBehaviour_SL_bb(i,3)==0.14 && AllBehaviour_SL_bb(i,9)==1
                selected_trials_1(i)=1;
            elseif AllBehaviour_SL_bb(i,8)==2 && AllBehaviour_SL_bb(i,3)==0.14 && AllBehaviour_SL_bb(i,9)==2
                selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_SL_bb(i,5)==3 || AllBehaviour_SL_bb(i,5)==5
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_SL_bb)
            %select trials
            if AllBehaviour_SL_bb(i,8)==1 && AllBehaviour_SL_bb(i,3)==0.07 && AllBehaviour_SL_bb(i,9)==1
                selected_trials_11(i)=1;
            elseif AllBehaviour_SL_bb(i,8)==2 && AllBehaviour_SL_bb(i,3)==0.07 && AllBehaviour_SL_bb(i,9)==2
                selected_trials_22(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_SL_bb(i,5)==3 || AllBehaviour_SL_bb(i,5)==5
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        ...
        avMB1(:,:) = mean(STFT(:,:,trl1),3);% correct-left
        avMB2(:,:) = mean(STFT(:,:,trl2),3);% correct-right
...
%wrong
h3=plot(Ts,(avMB11(ch(2),:)+avMB22(ch(1),:))/2,'b'); % contralateral to correct side
h4=plot(Ts,(avMB11(ch(1),:)+avMB22(ch(2),:))/2,'--b');%
