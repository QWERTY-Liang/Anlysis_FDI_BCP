...
[tnd,d,b,y] = deal(pm(1:2),pm(3:6),pm(7:8),pm(9:10));
%斜率控制
...
for c=1:4 % condition
    for n=1:N(c)
        ...
        for t=tnd(ceil(c/2)):dt:maxRT    % only simulate up to max 1.5 sec because that's the hard deadline in this task, anything later is a miss
            DV1 = DV1 + d(c)*dt + s*randn*sqrt(dt);    % note d and s are scaled by dt and square root of dt, which is the convention            
            %这里单独控制 第一和第二 ceil(c/2)
            bnd = b(ceil(c/2))-y(ceil(c/2))*t; % collapsing bound value at time t
            if DV1>bnd | DV1<-bnd
                RT(k,1) = t; 
