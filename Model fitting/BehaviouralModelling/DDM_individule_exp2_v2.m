...
figure (1); hist(data_allsubj(:,3),[0:.001:5]); xlim([-0.1 5]); xlabel('RT') % note 1-ms resolution

miss = find(data_allsubj(:,3)>maxRT);
data_allsubj(miss,2)=maxRT;
data_allsubj(miss,3)=nan;
% figure (2); hist(data_allsubj(:,3),[0:.001:5]); xlim([-0.1 3]); xlabel('RT') % note 1-ms resolution

miss = find(data_allsubj(:,3)<minRT/2);
