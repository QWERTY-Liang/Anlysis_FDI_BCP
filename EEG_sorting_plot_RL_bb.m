...
for sub = exp.sub_id(1:end)
     % % Load converted data
    EEG = pop_loadset([exp.finalpath 'csd_abb_cICAri'  'RL_' exp.filterLab 'aac' exp.name num2str(sub) '.set']);
...
filename = ['RL_bb' exp.name 'alldata'];
...
selected_trials_1 = zeros(length(AllBehaviour_RL_bb),1);  % example vector
selected_trials_2 = zeros(length(AllBehaviour_RL_bb),1);  % example vector
count=0;
% switch
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_RL_bb)
            %select trials
            if AllBehaviour_RL_bb(i,8)==AllBehaviour_RL_bb(i,9)
                selected_trials_1(i)=1;
            elseif AllBehaviour_RL_bb(i,8)~=AllBehaviour_RL_bb(i,9)
...
for i=1:length(AllBehaviour_RL_bb)
    ...
    if AllBehaviour_RL_bb(i,3)==0.14
        selected_trials_1(i)=1;
    elseif AllBehaviour_RL_bb(i,3)==0.07
        selected_trials_2(i)=1;
    end
