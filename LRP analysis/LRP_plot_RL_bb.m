...
addpath(genpath(exp.finalpath));
 EEG = pop_loadset([exp.finalpath 'RL_bbTLalldata.set']);
 load TL_AllBehaviour_RL_bb.mat 
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% channel_to_plot = 19;
left_channel = 114;  % C3通道号
right_channel = 53; 

selected_trials_1 = zeros(length(AllBehaviour_RL_bb),1);  % left C4-C3
selected_trials_2 = zeros(length(AllBehaviour_RL_bb),1);  % right C3-C4
count=0;
% switch%%%%%%%%%%calculate correct first
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_RL_bb)
            %select trials
            if AllBehaviour_RL_bb(i,8)==1 && AllBehaviour_RL_bb(i,9)==1
                selected_trials_1(i)=1;
            elseif AllBehaviour_RL_bb(i,8)==2 && AllBehaviour_RL_bb(i,9)==2
                selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_RL_bb(i,5)==3 || AllBehaviour_RL_bb(i,5)==5
...
LRP_right = (ERP_right(right_channel, :) - ERP_right(left_channel, :)) / 2;%这里先平均再相减
%ERP = mean(EEG_selected.data, 3);  % averaging across the third dimension (trials)
% Plot the ERP for a specific channel
...
num_plots = 10;
time_points = linspace(-1200, 230, num_plots);  % in milliseconds
...
for i = 1:num_plots
    ...
    topoplot(ERP_left(:, time_indices(i))-ERP_right(:, time_indices(i)), EEG_selected_left.chanlocs, 'maplimits', [-0.1*max(abs(ERP_left(:))), 0.1*max(abs(ERP_left(:)))]);
...
% 
% % Find the indices of the defined time points
% time_indices = arrayfun(@(t) find(EEG_selected_right.times >= t, 1), time_points);
% 
% % Plot scalp topographies
% figure;
% for i = 1:num_plots
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_RL_bb)
            %select trials
            if AllBehaviour_RL_bb(i,8)==1 && AllBehaviour_RL_bb(i,9)==2
                selected_trials_1(i)=1;
            elseif AllBehaviour_RL_bb(i,8)==2 && AllBehaviour_RL_bb(i,9)==1
                selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_RL_bb(i,5)==3 || AllBehaviour_RL_bb(i,5)==5
...
% plot LRPs
figure;
h1=plot(time_vector_left, LRP_left_correct,'r');
hold on;
h2=plot(time_vector_right, LRP_right_correct,'b');
h3=plot(time_vector_left, LRP_left,'--k');
h4=plot(time_vector_right, LRP_right,'--g');

legend([h1,h2,h3,h4],{'Left correct','Right correct','Left wrong','Right wrong'},'AutoUpdate', 'off');
xlabel('Time (ms)');
ylabel('Amplitude (µV)');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 3.1 high vs low using C3 and C4
%For left-hand responses, C4 is the contralateral (right hemisphere) signal and C3 is the ipsilateral (left hemisphere) signal.
%For right-hand responses, C3 is the contralateral (left hemisphere) signal and C4 is the ipsilateral (right hemisphere) signal.
%LRP=Contra−Ipsi
% Define the selected trials vector
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_RL_bb)
            %select trials
            if AllBehaviour_RL_bb(i,8)==1 && AllBehaviour_RL_bb(i,3)==0.14
                selected_trials_1(i)=1;
            elseif AllBehaviour_RL_bb(i,8)==2 && AllBehaviour_RL_bb(i,3)==0.14
                selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_RL_bb(i,5)==3 || AllBehaviour_RL_bb(i,5)==5
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_RL_bb)
            %select trials
            if AllBehaviour_RL_bb(i,8)==1 && AllBehaviour_RL_bb(i,3)==0.07
                selected_trials_1(i)=1;
            elseif AllBehaviour_RL_bb(i,8)==2 && AllBehaviour_RL_bb(i,3)==0.07
                selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_RL_bb(i,5)==3 || AllBehaviour_RL_bb(i,5)==5
...
legend([h1,h2,h3,h4],{'Left 0.14','Right 0.14','Left 0.07','Right 0.07'},'AutoUpdate', 'off');
xlabel('Time (ms)');
ylabel('Amplitude (µV)');
title(['LRP for Channel C3:', num2str(left_channel),' and C4: ', num2str(right_channel)]);
% Add vertical lines at specified time points
...
%% 3.1 FDI vs BCP using C3 and C4
%For left-hand responses, C4 is the contralateral (right hemisphere) signal and C3 is the ipsilateral (left hemisphere) signal.
%For right-hand responses, C3 is the contralateral (left hemisphere) signal and C4 is the ipsilateral (right hemisphere) signal.
%LRP=Contra−Ipsi
% Define the selected trials vector
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_RL_bb)
            %select trials
            if AllBehaviour_RL_bb(i,8)==1 && AllBehaviour_RL_bb(i,4)==1 &&AllBehaviour_RL_bb(i,9)==1%&& AllBehaviour_RL_bb(i,3)==0.14
                selected_trials_1(i)=1;
            elseif AllBehaviour_RL_bb(i,8)==2 && AllBehaviour_RL_bb(i,4)==1 && AllBehaviour_RL_bb(i,9)==2%&& AllBehaviour_RL_bb(i,3)==0.14
            selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_RL_bb(i,5)==3 || AllBehaviour_RL_bb(i,5)==5
...
for i = 1:num_plots
    subplot(2, 5, i);  % create a 2x4 subplot
    topoplot(ERP_left(:, time_indices(i))-ERP_right(:, time_indices(i)), EEG_selected_left.chanlocs, 'maplimits', [-0.2*max(abs(ERP_left(:))), 0.2*max(abs(ERP_left(:)))]);
    title([num2str(time_points(i)), ' ms']);
    colorbar;
end
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_RL_bb)
            %select trials
            if AllBehaviour_RL_bb(i,8)==1 && AllBehaviour_RL_bb(i,4)==2&&AllBehaviour_RL_bb(i,9)==1&& AllBehaviour_RL_bb(i,3)==0.14
                selected_trials_1(i)=1;
            elseif AllBehaviour_RL_bb(i,8)==2 && AllBehaviour_RL_bb(i,4)==2 &&AllBehaviour_RL_bb(i,9)==2&& AllBehaviour_RL_bb(i,3)==0.14
                selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_RL_bb(i,5)==3 || AllBehaviour_RL_bb(i,5)==5
...
legend([h1,h2,h3,h4],{'Left FDI','Right FDI','Left BCP','Right BCP'},'AutoUpdate', 'off');
xlabel('Time (ms)');
ylabel('Amplitude (µV)');
title(['LRP for Channel C3:', num2str(left_channel),' and C4: ', num2str(right_channel)]);
% Add vertical lines at specified time points
