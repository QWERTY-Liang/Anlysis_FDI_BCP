%%  4.1 EEG sortintg and plot for pre stimili SL  LRP  test not observious
% Author: Liang Tong 
% Date: 4/7/2024 
...
selected_trials_1 = zeros(length(AllBehaviour_SL_b),1);  % left C4-C3
selected_trials_2 = zeros(length(AllBehaviour_SL_b),1);  % right C3-C4
count=0;
% switch%%%%%%%%%%calculate correct first
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_SL_b)
            %select trials
            if AllBehaviour_SL_b(i,8)==1 && AllBehaviour_SL_b(i,9)==1
                selected_trials_1(i)=1;
            elseif AllBehaviour_SL_b(i,8)==2 && AllBehaviour_SL_b(i,9)==2
...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_SL_b)
            %select trials
            if AllBehaviour_SL_b(i,8)==1 && AllBehaviour_SL_b(i,9)==2
                selected_trials_1(i)=1;
            elseif AllBehaviour_SL_b(i,8)==2 && AllBehaviour_SL_b(i,9)==1
                selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_SL_b(i,5)==3 || AllBehaviour_SL_b(i,5)==5
