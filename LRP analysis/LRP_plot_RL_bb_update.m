...
%     case 'CW_SL_b' %correct vs wrong; pre cue SL
        for i=1:length(AllBehaviour_SL_bb)
            %select trials
            if AllBehaviour_SL_bb(i,9)==1 && AllBehaviour_SL_bb(i,4)==1
                selected_trials_1(i)=1;
            elseif AllBehaviour_SL_bb(i,9)==2 && AllBehaviour_SL_bb(i,4)==1
...
num_plots = 10;
time_points = linspace(-188*3.5, 188, num_plots);  % in milliseconds
...
for i = 1:num_plots
    ...
    topoplot(ERP_left(:, time_indices(i))-ERP_right(:, time_indices(i)), EEG_selected_left.chanlocs, 'maplimits', [-8,8]);
...
%% find all wrong
       for i=1:length(AllBehaviour_SL_bb)
            %select trials
            if AllBehaviour_SL_bb(i,9)==1 && AllBehaviour_SL_bb(i,4)==2
                selected_trials_1(i)=1;
            elseif AllBehaviour_SL_bb(i,9)==2 && AllBehaviour_SL_bb(i,4)==2
                selected_trials_2(i)=1;
            end
            %exclude invalide(too early or wrong muscle)
            if AllBehaviour_SL_bb(i,5)==3 || AllBehaviour_SL_bb(i,5)==5
