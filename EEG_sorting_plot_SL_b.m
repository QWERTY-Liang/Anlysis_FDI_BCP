%%  3. EEG sortintg and plot for pre evidence SL 
% Author: Liang Tong
% Date: 2/7/2024 
...
for sub = exp.sub_id(1:end)
     % % Load converted data
    EEG = pop_loadset([exp.finalpath 'csd_ab_cICAri'  'SL_' exp.filterLab 'aac' exp.name num2str(sub) '.set']);
...
filename = ['SL_b' exp.name 'alldata'];
...
for i=flip(1:length(AllBehaviour_new)) %倒序保证删除准确
 ...
 % save as .mat file   
    save([exp.finalpath exp.name '_AllBehaviour_SL_b' ],'AllBehaviour_SL_b');


%% 3. Sorting v%% pre evidence SL
% col 1: subject num
% col 2: block num
